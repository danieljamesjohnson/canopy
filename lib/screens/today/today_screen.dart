  ...
  Widget _buildPositionedRow(
    ...
    switch (row) {
      ...
      case ChunkRow(:final chunk, :final isLive):
        ...
        // Phase 32 (TAPBREAK-02, D-32-01): the break density ternary
        // collapses to the same two-way shape the work ternary below it
        // already uses. Phase 29's third `subCompact` band is retired
        // outright, not re-thresholded — at kPixelsPerMinute = 6.0 a
        // 5-minute break's 30dp slot would still have landed under the old
        // 32dp `kSubCompactBreakMinHeight` threshold, reproducing the exact
        // hairline defect this phase exists to remove, had the threshold
        // simply moved with the scale instead of being deleted along with
        // the tier it gated. The `compact` tier below is a real bordered
        // Card with a Skip rail now (TAPBREAK-01/03), not a hairline. The
        // work branch below is UNCHANGED (byte-identical).
        final density = isBreak
            ...
            : (slot >= kFullTierMinHeight
                  ? ChunkCardDensity.full
                  : ChunkCardDensity.compact);
        // Phase 32 (TAPBREAK-01, D-32-02): the break arm collapses into
        // exactly the shape the work arm immediately below already uses —
        // no enlarged hit-test envelope, no `visualHeight` confinement
        // argument. Phase 31's grown-`Positioned`/slop mechanism existed
        // solely to widen a break's `Dismissible` hit-test box beyond its
        // painted slot; breaks have no swipe target to widen any more
        // (button-only, D-32-02), so a break's `Positioned` is now
        // structurally identical to a work chunk's — the shared return
        // below handles both, differing only in which `ChunkCardDensity`
        // the ternary above picked and which card `_buildChunkCard`
        // constructs (`SwipeableChunkCard`'s restored early return sends a
        // break straight to `ChunkCard`, never through `Dismissible`).
        //
        // PD-10: ClipRect + OverflowBox is a safety net, not a min/max
