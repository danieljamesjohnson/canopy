...
void main() {
  group('GoalCard valence badge (ENERGY-04a)', () {
    testWidgets('shows "Gives" badge + bolt icon when energyValence == gives', (
      tester,
    ) async {
      await pumpWithMood(
        tester,
      ...
      expect(
        ...
        findsOneWidget,
        reason:
            'Valence badge must show "Gives" text for gives valence — ENERGY-04a',
      );
      expect(
        find.byIcon(Icons.bolt),
        findsOneWidget,
        reason:
            'Valence badge must show bolt icon for gives valence — ENERGY-04a',
      );
    });
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          ...
          findsOneWidget,
          reason:
              'Valence badge must show "Costs" text for costs valence — ENERGY-04a',
        );
        expect(
          find.byIcon(Icons.hourglass_empty),
          findsOneWidget,
          reason:
              'Valence badge must show hourglass icon for costs valence — ENERGY-04a',
        );
      },
    );

    testWidgets('shows NO valence badge when energyValence == neutral', (
      tester,
    ) async {
      await pumpWithMood(
        tester,
  ...
  group('GoalCard emoji tag (ENERGY-03b)', () {
    testWidgets('emoji appears in title row when emojiTag is set', (
      tester,
    ) async {
      const knownEmoji = '⚡';
      await pumpWithMood(
        tester,
        GoalCard(goal: _stubGoal('g1', 'Running', emojiTag: knownEmoji)),
      );
    ...
    testWidgets('no emoji appears when emojiTag is null', (tester) async {
      await pumpWithMood(
        tester,
        GoalCard(goal: _stubGoal('g1', 'No Emoji Goal')),
      );
    ...
    testWidgets('emoji + gives badge both render together', (tester) async {
      const knownEmoji = '🌟';
      await pumpWithMood(
        tester,
        GoalCard(
