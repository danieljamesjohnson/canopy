// Widget tests for BreathingPulseCta — Phase 6 Plan 06 Task 3. Renamed in
// Phase 22 Plan 04 (from home_screen_breathing_pulse_test.dart) to match its
// subject: the widget moved from home_screen.dart to
// lib/screens/today/widgets/breathing_pulse_cta.dart in Plan 22-03, when the
// unified Today screen absorbed the old Home screen's empty-state CTA.
//
...
//
// Per W-3 we pump `BreathingPulseCta` directly (a standalone public widget)
// so the test does not need TodayScreen's full provider tree.
//
// The reduced-motion path reads from
// `WidgetsBinding.instance.platformDispatcher.accessibilityFeatures.disableAnimations`
// (see breathing_pulse_cta.dart's `_animationsDisabled` getter). Tests
// configure that via `tester.platformDispatcher.accessibilityFeaturesTestValue`
// — the canonical Flutter test path; `MediaQueryData(disableAnimations: true)`
...
void main() {
  group('BreathingPulseCta', () {
    testWidgets('animates when enabled=true (blur changes across frames)', (
      tester,
    ) async {
