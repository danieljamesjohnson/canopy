...
status: planned
current_phase_next: "Phase 36 is PLANNED, not executed — run /gsd-execute-phase 36. Waves 1-4 run on danserver; wave 5 (36-07) is the owner's MacBook."
stopped_at: "PHASE 36 PLANNED AND READY TO EXECUTE (7 plans, 5 waves). Run /gsd-execute-phase 36. Waves 1-4 are fully runnable here because 36-01 builds injected seams for every external call; wave 5 (36-07) is a blocking-human gate on the owner's MacBook and has NO verify block by design. ALL owner rulings are already taken — see 36-DECISIONS.md, do not re-ask: D-36-01 native iOS not browser; D-36-02 bundle id renamed; D-36-03 both iOS sources coexist with a REQUIRED overlap-warning mitigation. The Google Cloud project, consent screen, iOS OAuth client and real client ID (.google-client-id, gitignored) all EXIST. || PHASE 35 IS STILL OPEN at two owner gates and is NOT superseded by 36: the browser UAT at http://danserver:8161/ (35-UAT.md, fixture regenerated by tools/gen-uat-calendar.py so it cannot go stale) and 35-05 Task 3's iOS device check. 36-07 deliberately carries 35-05's steps as its Section A so one sitting closes both. || CI NOW EXISTS (.github/workflows/ci.yml) and is the answer to 'danserver has no Xcode/Android SDK': iOS build PASSED on run 35864911239, confirming the com.danjjohnson.canopy rename compiles. It also caught a PRE-EXISTING broken Android build (missing core library desugaring) that had never been detectable here. || UNVERIFIED AND LOAD-BEARING: Phase 35 research Assumption A1 (device_calendar_plus returns expanded occurrences with exceptions applied) has never been checked on a device, and the correction that sign-in adds no recurrence benefit on iOS rests entirely on it."
last_updated: "2026-09-25T00:00:00.000Z"
last_activity: 2026-09-25
last_activity_desc: "Phase 36 scoped, researched (twice — transport pivoted to native iOS), pattern-mapped and planned; bundle id renamed repo-wide; GitHub CI added and a pre-existing broken Android build fixed"
state_head: 30b8601
progress:
  total_phases: 9
  completed_phases: 8
...
**Phase 36 is planned and unexecuted. Run `/gsd-execute-phase 36`.** Phase 35 is built but still
open at two owner gates — 36 does not supersede it.

### The five things a fresh agent most needs to know

1. **Every owner ruling in Phase 36 is already taken. Read `36-DECISIONS.md` and do not re-ask any
   of them.** Two of the three were decided *against* the recommendation, with the cost on screen —
   they are informed overrides, not oversights. D-36-03 in particular carries a **required
   mitigation**: both iOS calendar sources coexist and the user ticks per calendar, so ticking the
   same calendar twice doubles every meeting. The overlap must be made visible **at tick time**, and
   must **not** be silently de-duplicated.

2. **`36-RESEARCH.md` was written, then partly invalidated, then revised in place.** The transport
   pivoted from a browser PKCE flow to **native iOS** mid-phase. Sections are marked
   `REVISED — native iOS` or `UNCHANGED — transport-independent`, and superseded text is struck
   through but **retained**. Read the banner first. Do not build from struck-through material.

3. **Nothing in Phase 36 is end-to-end verifiable on danserver** — no Xcode, no Android SDK, ever.
   Waves 1–4 are testable here only because `36-01` builds injected seams for every external call.
   Wave 5 is the owner's MacBook, `gate="blocking-human"`, and has **no `<verify>` block by design**.
   Do not invent one. `35-05` is the worked precedent.

4. **CI exists now and is the answer to that constraint** (`.github/workflows/ci.yml`). GitHub's
   macOS runners have Xcode. The iOS build **passed** (run `35864911239`), which is how we know the
   `com.danjjohnson.canopy` rename compiles. On its first run CI also caught a **pre-existing broken
   Android build** — `flutter_local_notifications` needs core library desugaring, never enabled,
   undetectable here, and invisible to 821 green tests. Phase 36 adds `CFBundleURLTypes` and a native
   plugin; CI is what will catch it if that breaks the iOS build.

5. **The owner's Google Cloud setup is DONE.** Project, Calendar API, consent screen with
   `calendar.readonly`, himself as test user, and an **iOS** OAuth client bound to
   `com.danjjohnson.canopy`. The real client ID is at `.google-client-id` (gitignored, mode 600).
   Nothing further is needed from him to build.

### Phase 35 is still open — and one of its gates blocks part of 36

- **Browser UAT:** `35-UAT.md`, live at `http://danserver:8161/`. The fixture is generated by
  `tools/gen-uat-calendar.py` relative to today, **because the original hardcoded one silently rotted
  and would have shown an empty timeline** — a false failure manufactured by the test data.
- **iOS device check:** `35-05` Task 3. `36-07` deliberately carries its steps as **Section A**, so
  one sitting at the Mac closes both. This is not an economy: the double-tick warning in 36 cannot be
  tested unless the device calendar list works.

### Carried unverified, and load-bearing

**Phase 35 research Assumption A1** — that `device_calendar_plus.listEvents()` returns expanded
occurrences with exceptions applied — has **never been checked on a device**, and Phase 35's own
research calls it *"the single most load-bearing claim in the whole phase."* The correction on record
in `36-DECISIONS.md` (that Google sign-in adds no recurrence benefit on iOS) rests entirely on it. If
A1 is false, that correction is wrong and the Google path is more valuable than currently stated.

### Open `WINDOWS.md` entries

Entry 1 (`RECURRENCE-ID`/`EXDATE` unsupported on the ICS path — **proven**, not assumed) and entry 3
(pre-existing Hive fields without `defaultValue:`, same class as the crash Phase 35 fixed). Entry 2
is closed.
