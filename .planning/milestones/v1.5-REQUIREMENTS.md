# Requirements: Canopy — v1.5 "Right Now" (ARCHIVED)

**Status:** ✅ SHIPPED 2026-08-14 — 16/16 satisfied, all verified.
**Outcome:** every requirement delivered as written. One is worth noting: **NOW-01** was delivered
by Phase 24 as a `NowMarkerRow` list item, then re-delivered by Phase 26 as a `NowLineOverlay`
after 26 deleted the marker widget outright. Judged at the milestone audit as a *mechanism swap,
not a regression* — the overlay satisfies the requirement text more strongly, since it renders in
every `NowState` including `Active`, whereas Phase 24 had to suppress it mid-chunk to avoid
asserting a boundary position that wasn't true. Phase 26 removed the reason for the suppression
rather than working around it.

No requirement was dropped, deferred, or reinterpreted downward.
...
- v1.5 requirements: **16 total** (the "14" here predated Phase 26; CAL-01/02/03 were added when
  Phase 26 was inserted from Dan's Phase 24 UAT, and DEV-01/02/03 when Phase 25 was)
- Mapped to phases: 16 (100%)
- Unmapped: 0 ✓
- **Satisfied at ship: 16/16 ✓**
