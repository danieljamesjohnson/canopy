# Milestone v1.5: Right Now

**Status:** ✅ SHIPPED 2026-08-14
**Phases:** 21-26
**Total Plans:** 28 (26 with SUMMARY.md — two documented deviations, see below)
**Requirements:** 16/16 satisfied
**Tests:** 289 → 560 green · `flutter analyze` clean
**Commits:** 212 · 54 files, +11,060 / −3,690
**Timeline:** 2026-08-07 → 2026-08-14 (8 days)
...
Shipped. The app now has a single Today destination that renders the day as a **time-proportional
surface** with a **now-line at the true current moment**, including inside an activity's span. Break
cadence scales with the morning mood, a running break reads as rest rather than dead time, and no
copy anywhere frames the user as behind.

The milestone's defining characteristic: **five of its six phases rebuilt the same surface**
(`lib/screens/today/`), each layering onto or replacing the last. Phase 22 merged Home and Schedule
into it; 23 made it live; 24 added a now-marker; 26 deleted that marker and replaced the whole layout
with a proportional Stack. The invariant that made this survivable was established early and held
throughout: **one now-detector, one arithmetic authority.**

## Accomplishments

1. **One screen that tells the truth about now.** Home and Schedule merged into `/today` (shell went
   4 destinations → 3), and the screen names the current activity — including a running break —
   with a live countdown that switches to per-second inside the final minute.

2. **The day has a shape.** Row height is proportional to duration, so a long stretch of work and a
   thin gap are distinguishable without reading a single time. A now-line sits at the true current
   moment rather than snapping to the nearest chunk boundary.

3. **The engine stopped scolding.** Long-break cadence scales with the morning mood
   (`{1:2, 2:3, 3:4, 4:4, 5:5}`), and the time-target rationale's deficit branch became
   "Working toward N.Nh this week" — enforced by a repo-wide grep gate, not just a code change.

4. **A single now-detector, defended across five phases.** `resolveNowState` has exactly one
   definition and one call site. Phase 22 eliminated parallel detectors (a code review caught a
   third); every phase after it added assertions to keep that true. Phase 26 extended the same
   discipline to pixels: `TimelineGeometry` is the sole minute→pixel authority, so the rows, the
   hour axis, the now-line and the scroll target can never disagree about where a minute sits.

5. **A time machine, built one phase before it was needed.** Phase 25's `DevClock` is a debug-only
   *offset* (never a frozen instant, so time keeps flowing and a moving line stays observable),
   wired into every clock seam and impossible to leave on unnoticed. It paid for itself immediately:
   two Phase 26 defects were only visible in states a fresh profile never generates.

6. **Behaviour that matches how people actually work.** Completing a chunk early moves the following
   break to start now while keeping its original end — the reclaimed time becomes rest, and nothing
   downstream shifts.

## Process deviations — recorded, not smoothed over

- **Phase 23 plan 04 has no SUMMARY.md.** It was the human sign-off gate; it was discharged through
  `23-UAT.md` (4/4 tests passed, seven gaps raised and closed by 23-05..08) rather than a write-up.
- **Phase 25 has no PLAN.md.** It was scoped directly in an orchestrator prompt because Phase 24's
  UAT was blocked on it, and executed as a single unplanned SUMMARY. Recorded in
  `25-VERIFICATION.md` as a real gap in the planning trail.
- **Phase 22's final UAT item was closed as SUPERSEDED, not verified.** Phase 26-05 replaced the
  scroll mechanism it tested; nobody judged that 250ms ease-out and nobody now can.

## The honest lesson of this milestone

Phase 26 shipped **two defects behind green tests**, both the same shape — *an assertion that could
not fail*:

1. A regression test asserted non-collision against `ChunkCard` when the defect lived in
   `LiveRowCard` (the live row is full-bleed, so it has no gutter for the chip to hide in). Green
   from the start against a live bug — and the defect was reported as fixed when it wasn't.
2. The hour-axis test asserted widget *count* and label *text*, never a painted rect. The first and
   last hour labels were sheared on every render, and the now-line shared the root cause, clipping
   at exactly `PreStart` and `DayComplete` — the two states where nothing else locates "now". A real
   CAL-02 failure at the range edges.
...
**Corrective practice adopted mid-phase and recommended as standing:** every regression test must be
**observed RED against the unfixed code** before acceptance. That is how the second pair was caught
properly. Generalised rule: when the symptom is "X overlaps Y", enumerate every widget type that can
be Y before writing the assertion.

## Carried tech debt

| Item | Severity |
|------|----------|
| `kPixelsPerMinute = 5.5` is provisional — set from a `flutter test` measurement, the same harness class that produced a wrong `kGutterWidth` of 75. A 12-hour day is ~3960px. | medium |
| ~50px dead space below a *break* live row — structural, since the reservation is sized to the taller work variant. Measured, documented, accepted. | low |
| `21-`, `23-`, `24-VALIDATION.md` still at `status: draft` — joins the identical carried debt from phases 15/16/17. `26-VALIDATION.md` was signed off properly. | low |

Full audit: `v1.5-MILESTONE-AUDIT.md`. Requirements: `v1.5-REQUIREMENTS.md`.
